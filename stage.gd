...
func _on_spawn_timer_timeout():
	...
	if hardness < 20:
		hardness += 1
		get_node("spawn_timer").wait_time = get_node("spawn_timer").wait_time * 0.99
		print (get_node("spawn_timer").wait_time)
		get_node("ui/bar").value += 1
		get_node("ui/hard").text = "Hard: " + str(hardness)
